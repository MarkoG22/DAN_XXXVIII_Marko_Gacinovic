...
namespace Zadatak_1
{
    class Program
    {
        // autoresetevent for route selection, waiting for the file
        static AutoResetEvent auto = new AutoResetEvent(false);
        
        // barrier for starting thread at the same time
        ...
        static void Routes()
        {
            ...
            lock (locker)
            {
                try
                {
                    ...
                    // autoresetevent notifying that routes are in the list
                    auto.Set();
                }
                catch (Exception e)
                {
        ...
        static void RouteSelection()
        {
            lock (locker)
            {
                ...
                {
                    // autoresetevent waiting until routes are read from the file
                    auto.WaitOne(rnd.Next(3000));
                }
        ...
        {    
            ...
            // barrier for starting trucks at the same time, after the loading
